        ...
        if(sym_no > 0) {
            for(Elf_Xword i = 0; i < sym_no; ++i) {
                ...
                if((name == "version") && (p_rodata != nullptr)) {
                    ...
                    out << "Found VERSION = " << std::hex << version_id_from_hash << std::endl;
                ...
                if(!processor->process_symbol(
                       name.c_str(), static_cast<uint32_t>(value), bind, type, other)) {
                    std::cerr << "Failed to save symbol '" << name << "'!" << std::endl;
                }
            }
